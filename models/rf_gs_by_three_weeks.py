...
def rf_gs_by_three_weeks():
    ...
    parameters = {'n_estimators':[1000,1100,1200,1300], 'min_samples_split':[2,3], 'max_depth':[14,15,16]}
    rf = RandomForestRegressor()
    kf = KFold(n_splits=5, random_state=None, shuffle=True)
    loss = make_scorer(model_value.value_mode, greater_is_better=False)
    gs = GridSearchCV(rf, param_grid=parameters, cv=kf, refit=True, scoring=loss, n_jobs=-1)
    print train_x.shape
    print train_y.shape
    ...
    result.to_csv(params.OUTPUT_PATH + 'result_rf_gs_by_three_weeks.csv', index=False, header=False)
